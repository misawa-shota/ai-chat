...
- [ ] ストリーミング中断時の部分メッセージ保持
  - `reader.done = true` になったとき、"done" SSEイベントを受信せずに終了した場合でも `accumulated` が空でなければメッセージリストに追加する
- [ ] AIのMarkdownレスポンスのレンダリング
  - `react-markdown` などを使用して、コードブロック・箇条書き・太字などを適切に表示する

### セキュリティ・バリデーション
- [ ] メッセージ長の上限バリデーション追加
  - Zodスキーマに `z.string().min(1).max(2000)` 相当の制限を追加
  - フロントエンドにも文字数カウンター or 制限を追加
- [ ] APIレート制限の実装
  - sessionId または IP ベースで単位時間あたりのリクエスト数を制限
  - 例: 1分間に30回まで

### コード品質
- [ ] `ChatWindow.tsx` のメッセージkeyをindexから安定したIDに変更
  - `Message` 型に `id` フィールドを追加（`crypto.randomUUID()` 等）
  - `key={i}` → `key={msg.id}` に変更
- [ ] デバッグ用 `console.error` の整理
  - `lib/hono/index.ts` の `[DB]`/`[AI]` ログを本番向けに整理（ロガー導入 or 条件分岐）

### データ管理
- [ ] 古いセッションのクリーンアップ機能
  - 一定期間（例: 30日）アクセスのないセッション・メッセージをMongoDBから削除するバッチ処理
  - `DELETE /api/chat/history/:sessionId` エンドポイントの追加（「新しい会話」実装時に利用）

### テスト追加
- [ ] `ChatWindow` コンポーネントのテスト追加
  - メッセージ一覧の表示確認
  - ストリーミング中のローディングドット表示確認
  - 空状態のプレースホルダー表示確認
- [ ] ストリーミング中断時のエラーハンドリングテスト追加
